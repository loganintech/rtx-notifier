...
use crate::{error::NotifyError, Notifier, Config};
...
pub async fn get_notifier() -> Result<Notifier, NotifyError> {
    let mut file = File::open("./config.json").await.map_err(|e| NotifyError::ConfigLoad(e))?;

    let mut buf = String::new();
    file.read_to_string(&mut buf).await.map_err(|e| NotifyError::ConfigLoad(e))?;
    ...
    Ok(Notifier {
        ...
        twilio: twilio::Client::new(&config.application_config.twilio_account_id, &config.application_config.twilio_auth_token),
        config,
    })
}

pub async fn write_config(notifier: &mut Notifier) -> Result<(), NotifyError> {
    let mut file = File::create("./config.json").await.map_err(|_| NotifyError::ConfigUpdate)?;
